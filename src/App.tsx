...
const operationsDict = [
  ...
  { name: "כארי ישאג / שאגת הארי (איראן 2026)", start: new Date("2026-02-28"), end: new Date() }
];
...
export default function App() {
  ...
  const uavRoutes = useMemo(() => {
    ...
    "אזור בקעה": ["ארגמן", "אש קודש", "בית חגלה", "בקעות", "בתרונות", "גיתית", "גלגל", "החווה של אורי כהן", "החווה של יאיא", "החווה של מנחם", "החווה של עשהאל", "חוות גנות", "חוות הרשאש", "חוות יד השומר", "חוות עמיאל", "חוות פריאל", "חמדת", "חמרה", "ייט'ב", "יפית", "מבואות יריחו", "מחולה", "מכורה", "מעלה אפרים", "מצפור פצאל", "משואה", "משכיות", "נעמה", "נערן", "נתיב הגדוד", "עינות קדם", "פצאל", "צומת אלמוג", "קדם ערבה", "רועי", "רותם", "שדמות מחולה", "שלומציון", "תומר"],
    "אזור בקעת בית שאן": ["בית אלפא וחפציבה", "בית השיטה", "בית יוסף", "בית שאן", "גני חוגה", "גשר", "חמדיה", "טייבה בגלבוע", "טירת צבי", "ירדנה", "כפר גמילה מלכישוע", "כפר רופין", "מולדת", "מירב", "מנחמיה", "מסילות", "מעוז חיים", "מעלה גלבוע", "נוה איתן", "נווה אור", "ניר דוד", "עין הנצי'ב", "רוויה", "רחוב", "רשפים, שלוחות, שלפים", "שדה אליהו", "שדה נחום", "שדי תרומות", "תל תאומים"],
    "אזור בקעת הירדן": ["ייט''ב"],
    "אזור גולן": ["אזור תעשייה צמח", "נווה אטי''ב"],
    "אזור גולן דרום": ["אבני איתן", "אזור תעשייה בני יהודה", "אלוני הבשן", "אלי עד", "אלמגור", "אניעם", "אפיק", "אשדות יעקב", "בני יהודה וגבעת יואב", "גשור", "האון", "חד נס", "חמת גדר", "חספין", "יונתן", "כנף", "כפר חרוב", "מבוא חמה", "מיצר", "מסדה", "מעגן", "מעלה גמלא", "נאות גולן", "נוב", "נטור", "עין גב", "צמח", "קדמת צבי", "קצרין", "קצרין - אזור תעשייה", "קשת", "רמות", "רמת מגשימים", "שער הגולן", "תל קציר"],
    "אזור גולן צפון": ["אודם", "אורטל", "אל רום", "בוקעתא", "מג'דל שמס", "מסעדה", "מרום גולן", "נווה אטי'ב", "נמרוד", "עין זיוון", "עין קנייא", "קלע אלון", "רמת טראמפ", "שעל"],
    "אזור גליל עליון": ["אבו סנאן", "אור הגנוז", "אזור תעשייה בר-לב", "אזור תעשייה חצור הגלילית", "אזור תעשייה כרמיאל", "אזור תעשייה צ.ח.ר", "אזור תעשייה שער נעמן", "אחיהוד", "איילת השחר", "אליפלט", "אמירים", "אמנון", "אפק", "אשרת", "בוסתן הגליל", "ביריה", "בית ג'אן", "בית העלמין החדש עכו", "בית העמק", "בענה", "בר יוחאי", "ג'דידה מכר", "ג'וליס", "גדות", "גיתה", "דוב''ב", "דיר אל-אסד", "הר חלוץ", "הר-חלוץ", "חולתה", "חמדת ימים", "חצור הגלילית", "חרשים", "טובא זנגריה", "טל - אל", "טל-אל", "ינוח ג'ת", "ינוח-ג'ת", "יסוד המעלה", "יסעור", "ירכא", "כורזים ורד הגליל", "כחל", "כישור", "כליל", "כמון - כמאנה מזרחית", "כסרא סמיע", "כפר הנשיא", "כפר יאסיף", "כפר מסריק", "כפר שמאי", "כרכום", "כרמיאל", "לבון", "לוחמי הגטאות", "לפידות", "מג'דל כרום", "מגדל תפן", "מזרעה", "מחניים", "מירון", "מנחת מחניים", "מרכז אזורי מרום גליל", "מרכז אזורי רמת כורזים", "משמר הירדן", "נחף", "נס עמים", "נתיב השיירה", "סאג'ור", "ספסופה - כפר חושן", "עין אל אסד", "עין המפרץ", "עכו", "עכו - אזור תעשייה", "עכו - רמות ים", "עמוקה", "עמיעד", "עמקה", "פלך", "פרוד", "צורית גילון", "צפת - נוף כנרת", "צפת - עיר", "צפת - עכברה", "קדיתא", "קדרים", "ראמה", "ראש פינה", "רגבה", "שבי ציון", "שדה אליעזר", "שזור", "שייח' דנון", "שמרת", "שפר", "תובל"],
    "אזור גליל תחתון": ["אזור תעשייה טמרה", "אזור תעשייה קדמת גליל", "אלומות", "אפיקים", "ארבל", "אתר ההנצחה גולני", "בית זרע", "בית ירח", "גבעת אבני", "גינוסר", "דגניה א", "דגניה ב", "הודיות", "הזורעים", "ואדי אל חמאם", "חוקוק", "טבריה", "יבנאל", "כינרת מושבה", "כינרת קבוצה", "כפר זיתים", "כפר חיטים", "כפר כמא", "כפר נהר הירדן", "לביא", "לבנים", "מגדל", "מצפה", "מצפה אבי''ב", "נבי שועייב", "עוזייר", "ערב אל-נעים", "פוריה כפר עבודה", "פוריה נווה עובד", "פוריה עילית", "רביד", "רומאנה", "שדה אילן", "שרונה"],
    "אזור דן": ["אור יהודה", "אזור", "בני ברק", "בת ים", "בת-ים", "גבעת השלושה", "גבעת שמואל", "גבעתיים", "גני תקווה", "גת רימון", "הרצליה - מערב", "הרצליה - מרכז וגליל ים", "חולון", "יהוד מונוסון", "יהוד-מונוסון", "כפר סירקין", "כפר שמריהו", "מגשימים", "מעש", "מקווה ישראל", "מתחם גלילות", "מתחם פי גלילות", "סביון", "פארק אריאל שרון", "פתח תקווה", "קריית אונו", "רמת גן - מזרח", "רמת גן - מערב", "רמת השרון", "תל אביב - דרום העיר ויפו", "תל אביב - מזרח", "תל אביב - מרכז העיר", "תל אביב - עבר הירקון"],
    "אזור דרום הנגב": ["אבו קרינאת", "אבו תלול", "אזור תעשייה דימונה", "אזור תעשייה רותם", "אל פורעה", "אשלים", "באר מילכה", "בחלק מהאזורים בארץ", "ביר הדאג'", "דימונה", "ואדי אל נעם דרום", "חירן", "טללים", "ירוחם", "כמהין", "כסייפה", "כפר הנוקדים", "מדרשת בן גוריון", "מצפה רמון", "מרחב עם", "מרעית", "משאבי שדה", "ניצנה", "סעווה", "עזוז", "ערד", "ערערה בנגב", "קדש ברנע", "קסר א-סר", "רביבים", "רתמים", "שאנטי במדבר", "שדה בוקר", "תל ערד"],
    "אזור הכרמל": ["בית אורן", "בית סוהר קישון", "בית צבי", "בת שלמה", "גבע כרמל", "גבעת וולפסון", "דור", "דור, נחשולים", "דלית אל כרמל", "הבונים", "ימין אורד", "כרם מהר''ל", "כרם מהר'ל", "מאיר שפיה", "מגדים", "מרכז מיר''ב", "מרכז מיר'ב", "נווה ים", "נחשולים", "ניר עציון, ימין אורד", "עופר", "עין איילה", "עין הוד", "עין חוד", "עין כרמל", "עספיא", "עתלית", "פוריידיס", "צרופה"],
    "אזור המפרץ": ["אושה", "אזור תעשייה ניר עציון", "אזור תעשייה קריית ביאליק", "איבטין", "בית עלמין תל רגב", "הדר ועיר תחתית", "החותרים", "חיפה - בת גלים ק.אליעזר", "חיפה - כרמל", "חיפה - כרמל, הדר ועיר תחתית", "חיפה - מערב", "חיפה - מפרץ", "חיפה - נווה שאנן ורמות כרמל", "חיפה - קריית חיים ושמואל", "חיפה-מפרץ", "טירת כרמל", "יגור", "כפר ביאליק", "כפר גלים", "כפר המכבי", "כפר חסידים", "ניר עציון", "נשר", "קריית אתא", "קריית ביאליק", "קריית ים", "קריית מוצקין", "רכסים", "רמת יוחנן"],
    "אזור העמקים": ["אורנים", "אזור תעשייה אלון התבור", "אזור תעשייה מבואות הגלבוע", "אזור תעשייה ציפורית", "אחוזת ברק", "אילניה", "אכסאל", "אלון הגליל", "אלוני אבא", "אלונים", "בית לחם הגלילית", "בית קשת", "בית שערים", "בלפוריה", "בסמת טבעון", "גבעת אלה", "גבת", "גדעונה", "גזית", "גן נר", "גניגר", "דבוריה", "דברת", "דחי", "הושעיה", "היוגב", "הסוללים", "הרדוף", "זרזיר", "חג'אג'רה", "טמרה בגלבוע", "יזרעאל", "יפיע", "יפעת", "ישובי אומן", "ישובי יעל", "כדורי", "כעביה טבאש", "כפר ברוך", "כפר גדעון", "כפר החורש", "כפר ח'וואלד", "כפר יהושע", "כפר יחזקאל", "כפר כנא", "כפר מצר", "כפר קיש", "כפר תבור", "כפר תקווה", "מגדל העמק", "מגן שאול", "מוקיבלה", "מזרע", "מנשית זבדה", "מרחביה מושב", "מרחביה קיבוץ", "מרכז חבר", "משהד", "מתחם סקי גלבוע", "נאעורה", "נהלל", "נוף הגליל", "נופית", "נורית", "נין", "נצרת", "סואעד חמירה", "סולם", "סנדלה", "עדי", "עילוט", "עין דור", "עין הנצי''ב", "עין חרוד", "עין חרוד, תל יוסף", "עין מאהל", "עפולה", "ציפורי", "קבוצת גבע", "קריית טבעון - בית זייד", "קריית טבעון-בית זייד", "ראס עלי", "ריינה", "רם און", "רמת דוד", "רמת ישי", "רמת צבי", "רשפים", "שדה יעקב", "שדמות דבורה", "שיבלי אום אלג'נם", "שלוחות", "שלפים", "שמשית", "שער העמקים", "שריד", "תחנת רכבת כפר ברוך", "תל יוסף", "תל עדשים", "תמרת"],
    "אזור השפלה": ["אבו-גוש", "אזור תעשייה גדרה", "אזור תעשייה נשר - רמלה", "אחיסמך", "אחיעזר", "אירוס", "באר יעקב", "בית דגן", "בית חנן", "בית חשמונאי", "בית עובד", "בית עוזיאל", "בן שמן", "גאליה", "גזר", "גיבתון", "גינתון", "גן שורק", "גן שלמה", "גנות", "גני הדר", "גני יוחנן", "זיתן", "חולדה", "חמד", "יגל", "יד רמב''ם", "יד רמב'ם", "יסודות", "יציץ", "ישרש", "כפר ביל''ו", "כפר ביל'ו", "כפר בן נון", "כפר חב''ד", "כפר חב'ד", "כפר נוער בן שמן", "כפר שמואל", "כרמי יוסף", "לוד", "מזכרת בתיה", "מצליח", "משמר איילון", "משמר דוד", "משמר השבעה", "נטעים", "ניר צבי", "נס ציונה", "נען", "נצר חזני", "נצר סרני", "נתיב הל''ה", "סתריה", "עזריה", "עיינות", "פארק תעשיות פלמחים", "פדיה", "פתחיה", "צפריה", "קריית עקרון", "ראשון לציון - מזרח", "ראשון לציון - מערב", "רחובות", "רמות מאיר", "רמלה", "תעשיון צריפין"],
    "אזור ואדי ערה": ["אום אל פחם", "אום אל קוטוף", "אזור תעשייה רגבים", "אל עריאן", "אליקים", "באקה אל גרבייה", "ברטעה", "ג'ת", "גבעת ניל''י", "גבעת ניל'י", "גבעת עוז", "גלעד", "דליה", "חריש", "יקנעם המושבה והזורע", "יקנעם עילית", "כפר קרע", "מגל", "מדרך עוז", "מועאוויה", "מי עמי", "מייסר", "מעלה עירון", "מצפה אילן", "מצר", "משמר העמק", "עין אל סהלה", "עין אל-סהלה", "עין העמק", "עין השופט", "ערערה", "קיבוץ מגידו", "קציר", "רגבים", "רמות מנשה", "רמת השופט"],
    "אזור יהודה": ["אביגיל", "אדורה", "אדורים", "אזור תעשייה מישור אדומים", "אזור תעשייה מיתרים", "איבי הנחל", "אלון", "אלון שבות", "אליאב", "אלעזר", "אפקה", "אפרת", "אשתמוע", "בית הברכה", "בית חג'י", "בית חג'י", "בית יתיר", "ביתר עילית", "בני אדם", "בני דקלים", "בר כוכבא", "בת עין", "גבעות", "הגבעה הצהובה", "היישוב היהודי חברון", "הר גילה", "הר עמשא", "חוות ארץ האיילים", "חוות אשכולות", "חוות דרומא", "חוות הרועה העברי", "חוות טואמין", "חוות טליה", "חוות יויו", "חוות מדבר חבר", "חוות מור ואברהם", "חוות מלאכי אברהם", "חוות מנחם", "חוות מקנה יהודה", "חוות נחלת אבות", "חוות עדן", "חוות צאן קדר", "חוות קשואלה", "חוות תלם צפון", "טנא עומרים", "כפר אדומים", "כפר אלדד", "כפר עציון", "כרמי צור", "כרמי קטיף ואמציה", "כרמל", "מגדל עוז", "מיצד", "מעון", "מעלה אדומים", "מעלה חבר", "מעלה עמוס", "מעלה רחבעם", "מצפה זי'ו", "מצפה זי'ו", "מצפה חגית", "מצפה יאיר", "מצפה יריחו", "נגוהות", "נווה דניאל", "נופי פרת", "נוקדים", "נטע", "סוסיא", "סוסיא הקדומה", "עוז וגאון", "עלמון", "עשהאל", "עתניאל", "פארק תעשיות מגדל עוז", "פני קדם", "צומת הגוש", "קדר", "קדר דרום", "קרית ארבע", "ראש צורים", "שדה בועז", "שדה בר", "שומריה", "שמעה", "שני ליבנה", "שקף", "תלם", "תקוע", "תקוע ד' וה", "תקוע ד' וה'"],
    "אזור ים המלח": ["אבנת", "אלמוג", "בית הערבה", "בתי מלון ים המלח", "ורד יריחו", "חוף קליה", "מלונות ים המלח מרכז", "מצדה", "מצוקי דרגות", "מצפה מדרג", "מצפה שלם", "מרחצאות עין גדי", "מרכז אזורי מגילות", "נאות הכיכר", "נווה זוהר", "עין בוקק", "עין גדי", "עין תמר", "קליה"],
    "אזור ירושלים": ["אבן ספיר", "אורה", "בית זית", "גבעון החדשה", "גבעת זאב", "ירושלים - אזור תעשייה עטרות", "ירושלים - דרום", "ירושלים - כפר עקב", "ירושלים - מזרח", "ירושלים - מערב", "ירושלים - מרכז", "ירושלים - צפון", "ירושלים - צפון ואלונים", "מבשרת ציון", "מוצא עילית", "עמינדב", "פנימיית עין כרם"],
    "אזור ירקון": ["אזור תעשייה אפק ולב הארץ", "אזור תעשייה חבל מודיעין שוהם", "איירפורט סיטי", "אלעד", "בארות יצחק", "בית נחמיה", "בית עריף", "בני עטרות", "ברקת", "גבעת כ''ח", "גבעת כ'ח", "גמזו", "חדיד", "חשמונאים", "טירת יהודה", "כפר דניאל", "כפר האורנים", "כפר טרומן", "כפר רות", "לפיד", "מבוא חורון", "מבוא מודיעים", "מודיעין - ישפרו סנטר", "מודיעין - ליגד סנטר", "מודיעין מכבים רעות", "מודיעין עילית", "מזור", "מתתיהו", "נאות קדומים", "נוף איילון", "נוף איילון, שעלבים", "נופך", "נחלים", "נחשונים", "עינת", "ראש העין", "רינתיה", "שוהם", "שילת", "שעלבים"],
    "אזור לכיש": ["אביגדור", "אבן שמואל", "אורות", "אזור תעשייה באר טוביה", "אזור תעשייה כנות", "אזור תעשייה עד הלום", "אזור תעשייה קריית גת", "אזור תעשייה רבדים", "אזור תעשייה תימורים", "אחווה", "אחוזם", "איתן", "אל עזי", "אלומה", "אמונים", "אשדוד - א", "אשדוד - א,ב,ד,ה", "אשדוד - אזור תעשייה צפוני ונ", "אשדוד - איזור תעשייה צפוני", "אשדוד - ג", "אשדוד - ג,ו,ז", "אשדוד - ח", "אשדוד - ח,ט,י,יג,יד,טז", "אשדוד - יא", "אשדוד -יא", "אשדוד -יא,יב,טו,יז,מרינה,סיטי", "אשדוד סיטי", "אשקלון", "ב", "באר טוביה", "ביצרון", "בית אלעזרי", "בית גמליאל", "בית חלקיה", "בית עזרא", "בן זכאי", "בני דרום", "בני עי''ש", "בני עי'ש", "בני ראם", "בניה", "גבעת ברנר", "גבעת וושינגטון", "גבעתי", "גדרה", "גן הדרום", "גן יבנה", "גני טל", "גת", "ד", "ה", "ו", "ורדון", "ז", "זבדיאל", "זוהר", "זרחיה", "חפץ חיים", "חצב", "חצור", "ט", "טו", "טז", "י", "יב", "יבנה", "יג", "יד", "יד בנימין", "יד נתן", "יז", "ינון", "כנות", "כפר אביב", "כפר אחים", "כפר הנגיד", "כפר הרי''ף וצומת ראם", "כפר הרי'ף וצומת ראם", "כפר ורבורג", "כפר מרדכי", "כרם ביבנה", "כרמי גת", "לכיש", "מישר", "מנוחה", "מעון צופיה", "מרינה", "מרכז שפירא", "משגב דב", "משואות יצחק", "מתחם בני דרום", "נגבה", "נהורה", "נוגה", "נווה מבטח", "נועם", "נחלה", "ניר בנים", "ניר גלים", "ניר ח''ן", "ניר ח'ן", "סגולה", "סיטי", "עוזה", "עוצם", "עזר", "עזריקם", "עין צורים", "ערוגות", "עשרת", "פארק תעשייה ראם", "פלמחים", "קבוצת יבנה", "קדמה", "קדרון", "קוממיות", "קריית גת", "קריית גת, כרמי גת", "קריית מלאכי", "רבדים", "רווחה", "שדה דוד", "שדה יואב", "שדה משה", "שדה עוזיהו", "שדמה", "שחר", "שלווה", "שפיר", "שתולים", "תחנת רכבת קריית מלאכי - יואב", "תימורים", "תלמי יחיאל", "תלמים"],
    "אזור מנשה": ["אביאל", "אור עקיבא", "אזור תעשייה יקנעם עילית", "אזור תעשייה מבוא כרמל", "אזור תעשייה קיסריה", "אלוני יצחק", "בי'ס כרמים בנימינה", "בית חנניה", "בנימינה", "ברקאי", "ג'סר א-זרקא", "גבעת עדה", "גן השומרון", "גן שמואל", "זכרון יעקב", "חדרה - מזרח", "חדרה - מערב", "חדרה - מרכז", "חדרה - נווה חיים", "כפר גליקסון", "כפר פינס", "להבות חביבה", "מאור", "מעגן מיכאל", "מעיין צבי", "מענית, גבעת חביבה", "משמרות", "מתחם שביל התפוזים", "נמל קיסריה", "עין עירון", "עין שמר", "עמיקם", "פרדס חנה כרכור", "קיסריה", "שדה יצחק", "שדות ים", "שער מנשה", "תחנת רכבת כפר יהושוע", "תלמי אלעזר"],
    "אזור מערב הנגב": ["אופקים", "אורים", "אזור תעשייה נ.ע.מ", "אשבול", "אשל הנשיא", "בטחה", "בית הגדי", "ברור חיל", "ברוש", "גבולות", "גילת", "דורות", "זרועה", "יושיביה", "מבועים", "מסלול", "מעגלים, גבעולים, מלילות", "ניר משה", "ניר עקיבא", "נתיבות", "פדויים", "פטיש", "פעמי תש'ז", "צאלים", "קלחים", "קריית חינוך מרחבים", "רוחמה", "רנן", "שבי דרום", "שדה צבי", "שיבולים", "שרשרת", "תאשור", "תדהר", "תלמי ביל'ו", "תפרח"],
    "אזור מערב לכיש": ["אזור תעשייה הדרומי אשקלון", "אזור תעשייה צפוני אשקלון", "אשקלון - דרום", "אשקלון - צפון", "באר גנים", "בית שקמה", "ברכיה", "בת הדר", "גיאה", "הודיה", "חלץ", "כוכב מיכאל", "כפר סילבר", "מבקיעים", "משען", "ניצן", "ניצנים", "ניר ישראל", "תלמי יפה"],
    "אזור מרכז הגליל": ["אבטליון", "אזור תעשייה תרדיון", "אעבלין", "אשבל", "אשחר", "בועיינה-נוג'ידאת", "ביר אלמכסור", "בית סוהר צלמון", "בית רימון", "דיר חנא", "דמיידה", "הררית יחד", "חוסנייה", "חזון", "חנתון", "טורעאן", "טמרה", "טפחות", "יובלים", "יודפת", "יעד", "כאבול", "כאוכב אבו אלהיג'א", "כלנית", "כפר חנניה", "כפר מנדא", "לוטם וחמדון", "מורן", "מורשת", "מנוף", "מסד", "מע'אר", "מעלה צביה", "מצפה אבי'ב", "מצפה נטופה", "מרכז אזורי משגב", "סכנין", "סלמה", "עוזייר, רומאנה", "עילבון", "עצמון - שגב", "עראבה", "ערב אל נעים", "קורנית", "ראס אל-עין", "רומת אל הייב", "רקפת", "שורשים", "שכניה", "שעב", "שפרעם"],
    "אזור מרכז הנגב": ["אום בטין", "אזור תעשייה עידן הנגב", "אל סייד", "אשכולות", "אתר דודאים", "באר שבע - דרום", "באר שבע - מזרח", "באר שבע - מערב", "באר שבע - צפון", "בית קמה", "גבעות בר", "דביר", "חורה", "חצרים", "כרמים", "כרמית", "להב", "להבים", "לקיה", "מיתר", "משמר הנגב", "נבטים", "סנסנה", "עומר", "רהט", "שגב שלום", "שובל", "תארבין", "תל שבע"],
    "אזור נגב": ["אבו קרינאת והפזורה", "אבו-תלול והפזורה", "אום בטין והפזורה", "אל-סייד והפזורה", "ביר הדאג", "גבעולים", "כסייפה והפזורה", "לקיה והפזורה", "מלילות", "מעגלים", "פעמי תש''ז", "רהט והפזורה", "שגב שלום והפזורה", "תלמי ביל''ו"],
    "אזור עוטף עזה": ["אבשלום", "אוהד", "אור הנר", "איבים", "ארז", "בארי", "בני נצרים", "גבים", "גבים, מכללת ספיר", "גברעם", "דקל", "זיקים", "זמרת", "זמרת, שובה", "חולית", "חוף זיקים", "חניון רעים אנדרטת הנובה", "יבול", "יד מרדכי", "יכיני", "ישע", "יתד", "כיסופים", "כפר מימון ותושיה", "כפר עזה", "כרם שלום", "כרמיה", "מבטחים", "מבטחים, עמיעוז, ישע", "מגן", "מטווח ניר עם", "מכללת ספיר", "מפלסים", "נווה", "נחל עוז", "ניר יצחק", "ניר עוז", "ניר עם", "נירים", "נתיב העשרה", "סופה", "סעד", "עין הבשור", "עין השלושה", "עלומים", "עמיעוז", "פרי גן", "צוחר", "צוחר, אוהד", "רעים", "שדה ניצן", "שדי אברהם", "שדרות", "שדרות, איבים", "שובה", "שוקדה", "שלומית", "תלמי אליהו", "תלמי יוסף", "תקומה"],
    "אזור ערבה": ["אל מסק", "אל עמארני", "אל עמארני, אל מסק", "אליפז ומכרות תמנע", "באר אורה", "גרופית", "חצבה", "יהל", "יטבתה", "לוטן", "נאות סמדר", "נווה חריף", "סמר", "ספיר", "עידן", "עין חצבה", "עין יהב", "פארן", "צופר", "צוקים", "קטורה", "שחרות", "שיטים"],
    "אזור קו העימות": ["אביבים", "אבירים", "אבן מנחם", "אדמית", "אזור תעשייה רמת דלתון", "איזור תעשייה מילואות צפון", "אילון", "אלקוש", "בית הלל", "בית העלמין החדש נהריה", "בית ספר שדה מירון", "בן עמי", "בצת", "ברעם", "ג'ש - גוש חלב", "גונן", "גורן", "גורנות הגליל", "געתון", "גשר הזיו", "דוב'ב", "דישון", "דלתון", "דפנה", "הגושרים", "הילה", "זרעית", "חוסן", "חוף אכזיב", "חוף בצת", "חורפיש", "חניתה", "יחיעם", "יערה", "יפתח", "יראון", "כברי", "כפר בלום", "כפר גלעדי", "כפר ורדים", "כפר יובל", "כפר סאלד", "כרם בן זמרה", "לב החולה", "להבות הבשן", "לימן", "מטולה", "מכמנים - כמאנה מערבית", "מלכיה", "מנות", "מנרה", "מעונה", "מעיין ברוך", "מעיליא", "מעלות תרשיחא", "מצובה", "מרגליות", "מרכז אזורי מבואות חרמון", "משגב עם", "מתת", "נאות מרדכי", "נהריה", "נווה זיו", "נטועה", "סאסא", "סער", "ע'ג'ר", "עבדון", "עברון", "עין יעקב", "עלמה", "עמיר", "ערב אל עראמשה", "פסוטה", "פקיעין", "פקיעין החדשה", "צבעון", "צוריאל", "קיבוץ דן", "קריית שמונה", "ראש הנקרה", "ריחאנייה", "רמות נפתלי", "שאר ישוב", "שדה נחמיה", "שומרה", "שלומי", "שמיר", "שניר", "שתולה", "תל חי"],
    "אזור שומרון": ["אביתר", "אבני חפץ", "אזור תעשייה אריאל", "אזור תעשייה בראון", "אזור תעשייה ברקן", "אזור תעשייה שחק", "אזור תעשייה שער בנימין", "אחיה", "איתמר", "אלון מורה", "אעירה השחר", "אריאל", "בית אל", "בית אריה", "בית חורון", "ברוכין", "ברקן", "גבע בנימין", "גבעת אסף", "גבעת הראל", "גבעת הרואה", "גבעת פורת יוסף", "גופנה", "דולב", "דורות עילית", "החווה של זוהר", "הר ברכה", "חוות אביה", "חוות אביחי", "חוות אל נווה", "חוות אלחי", "חוות בניהו", "חוות גלעד", "חוות חנינא", "חוות יאיר", "חוות ינון", "חוות מגדלים", "חוות מגנזי", "חוות מלכיאל", "חוות מעלה אהוביה", "חוות מרום שמואל", "חוות נווה צוף", "חוות נוף אב'י", "חוות נוף אב'י", "חוות נחל שילה", "חוות נחלת צבי", "חוות עולם חסד", "חוות צרידה", "חוות ראש תאנה", "חוות שדה", "חוות שוביאל", "חוות שחרית", "חומש", "חיננית", "חרמש", "חרמש דרום", "חרשה", "טל מנשה", "טלמון", "יבוא דודי", "יצהר", "יקיר", "כוכב השחר", "כוכב יעקב", "כפר תפוח", "כרם רעים", "מבוא דותן", "מגדלים", "מגרון", "מכינת אלישע", "מלאכי השלום", "מעלה לבונה", "מעלה מכמש", "מצפה דני", "נווה ארז", "נווה צוף", "נופי נחמיה", "נופים", "נחליאל", "נילי", "נעלה", "נריה", "עדי עד", "עופרים", "עטרת", "עלי", "עלי זהב - לשם", "עמיחי", "עמנואל", "ענב", "עפרה", "פדואל", "פסגות", "קדומים", "קידה", "קריית נטפים", "קרני שומרון", "רבבה", "רחלים", "ריחן", "רימונים", "רמת מגרון", "שבות רחל", "שבי שומרון", "שדה אפרים", "שילה", "שקד", "תל ציון"],
    "אזור שפלת יהודה": ["אבו גוש", "אביעזר", "אדרת", "אזור תעשייה ברוש", "אזור תעשייה הר טוב - צרעה", "אשתאול", "בית גוברין", "בית מאיר", "בית ניר", "בית נקופה", "בית שמש", "בקוע", "בר גיורא", "גבעות עדן", "גבעת יערים", "גבעת ישעיהו", "גיזו", "גלאון", "גפן", "הר אדר", "הראל", "זכריה", "זנוח", "טל שחר", "יד השמונה", "ישעי", "כסלון", "כפר אוריה", "כפר הנוער קריית יערים", "כפר זוהרים", "כפר מנחם", "לוזית", "לטרון", "מבוא ביתר", "מחסיה", "מטע", "מיני ישראל - נחשון", "מסילת ציון", "מעלה החמישה", "נווה אילן", "נווה מיכאל - רוגלית", "נווה שלום", "נחושה", "נחם", "נחשון", "נטף", "נס הרים", "נתיב הל'ה", "עגור", "עין נקובא", "עין ראפה", "צובה", "צור הדסה", "צלפון", "צפרירים", "צרעה", "קריית יערים", "קריית ענבים", "רטורנו - גבעת שמש", "רמת רזיאל", "שדות מיכה", "שואבה", "שורש", "שריגים - לי-און", "תירוש", "תעוז", "תרום"],
    "אזור שרון": ["אביחיל", "אבן יהודה", "אודים", "אורנית", "אזור תעשייה טירה", "אזור תעשייה כפר יונה", "אזור תעשייה עמק חפר", "אחיטוב", "אייל", "אליכין", "אלישיב", "אלישמע", "אלפי מנשה", "אלקנה", "אמץ", "ארסוף", "בארותיים", "בורגתה", "בחן", "בי'ס כרמים בנימינה", "בית ברל", "בית הלוי", "בית חרות", "בית יהושע", "בית ינאי", "בית יצחק - שער חפר", "בית עלמין מורשה", "ביתן אהרן", "בני דרור", "בני ציון", "בצרה", "בת חן", "בת חפר", "ג'לג'וליה", "גאולי תימן", "גאולים", "גבעת חביבה", "גבעת חיים איחוד", "גבעת חיים מאוחד", "גבעת חן", "גבעת שפירא", "גן חיים", "גן יאשיה", "גנות הדר", "גני עם", "געש", "הדר עם", "הוד השרון", "המעפיל", "המרכז האקדמי רופין", "העוגן", "זמר", "חבצלת השרון וצוקי ים", "חגור", "חגלה", "חופית", "חורשים", "חיבת ציון", "חניאל", "חרב לאת", "חרוצים", "חרות", "טייבה", "טירה", "יד חנה", "ינוב", "יעבץ", "יעבץ, יעף", "יעף", "יקום", "ירחיב", "ירקונה", "כוכב יאיר - צור יגאל", "כפר ברא", "כפר הס", "כפר הרא''ה", "כפר הרא'ה", "כפר ויתקין", "כפר חיים", "כפר ידידיה", "כפר יונה", "כפר מונש", "כפר מל''ל", "כפר מל'ל", "כפר נטר", "כפר סבא", "כפר עבודה", "כפר קאסם", "מכון וינגייט", "מכמורת", "מסוף אורנית", "מעברות", "מענית", "מרכז אזורי דרום השרון", "משמר השרון", "משמרת", "מתן", "נווה ימין", "נווה ירק", "נורדיה", "ניצני עוז", "ניר אליהו", "נירית", "נעורים", "נתניה - מזרח", "נתניה - מערב", "סלעית", "עדנים", "עולש", "עזריאל", "עין החורש", "עין ורד", "עין שריד", "עץ אפרים", "פורת", "פרדסיה", "צופים", "צופית", "צור יצחק", "צור משה", "צור נתן", "קדימה צורן", "קדימה-צורן", "קלנסווה", "רמות השבים", "רמת הכובש", "רעננה", "רשפון", "שדה ורבורג", "שדי חמד", "שושנת העמקים", "שער אפרים", "שערי תקווה", "שפיים", "תחנת רכבת ראש העין", "תל יצחק", "תל מונד", "תנובות"],
  };

  const popularRegions = Object.keys(regionToCities).sort((a,b) => a.localeCompare(b, 'he'));
...
loadData();
  ...
  useEffect(() => {
    ...
    // Count frequency of each consecutive city-pair across all routes
    const segmentFreq: Record<string, number> = {};
    uavRoutes.forEach(route => {
      for (let i = 0; i < route.length - 1; i++) {
        const key = `${route[i].cities}||${route[i + 1].cities}`;
        segmentFreq[key] = (segmentFreq[key] || 0) + 1;
      }
    });
    ...
    drawOrder.forEach((routeIdx) => {
      ...
      for (let i = 0; i < route.length - 1; i++) {
        const key = `${route[i].cities}||${route[i + 1].cities}`;
        const freq = segmentFreq[key] || 1;
        const t = freq / maxSegFreq; // 0–1
  ...
  useEffect(() => {
    ...
    if (compareMode && !compareOperation.includes('all')) {
      ...
      // dataZoom → filter date range (compare mode)
      if (timeResolution === 'date') {
        timeSeriesInstance.current.off('datazoom');
        timeSeriesInstance.current.on('datazoom', () => {
          const option = timeSeriesInstance.current!.getOption() as any;
          const dz = Array.isArray(option.dataZoom) ? option.dataZoom[0] : null;
          if (!dz) return;
          const startPct = dz.start ?? 0;
          const endPct = dz.end ?? 100;
          const startIdx = Math.round(startPct / 100 * (allKeys.length - 1));
          const endIdx = Math.round(endPct / 100 * (allKeys.length - 1));
          const startDate = allKeys[Math.max(0, startIdx)] || '';
          const endDate = allKeys[Math.min(allKeys.length - 1, endIdx)] || '';
      ...
      if (timeResolution === 'date') {
        ...
        timeSeriesInstance.current.on('datazoom', () => {
          ...
          const startPct = dz.start ?? 0;
          const endPct = dz.end ?? 100;
          const startIdx = Math.round(startPct / 100 * (xData.length - 1));
          const endIdx = Math.round(endPct / 100 * (xData.length - 1));
          const startDate = xData[Math.max(0, startIdx)] || '';
          const endDate = xData[Math.min(xData.length - 1, endIdx)] || '';
          if (startDate && endDate) {
            setDateRange({ start: startDate, end: endDate });
          }
        });
